...
const atualizarPaineis = (cpf) => {
  const atualizarPainel = (tbody, lancamentosProcessados) => {
    ...
    for (const lancamento of lancamentosProcessados) {
      ...
      td.innerHTML = (Math.round(lancamento.valor * 100) / 100)
        .toFixed(2)
        .replace(",", "")
        .replace(".", ",");
  ...
  atualizarPainel(
    document.getElementById("saldosPorConta"),
    recuperarSaldosPorConta(lancamentos)
  );
  atualizarPainel(
    document.getElementById("ultimosLancamentos"),
    recuperarMaiorMenorLancamentos(cpf, lancamentos)
  );
  atualizarPainel(
    document.getElementById("maioresSaldos"),
    recuperarMaioresSaldos(lancamentos)
  );
  atualizarPainel(
    document.getElementById("maioresMedias"),
    recuperarMaioresMedias(lancamentos)
  );
};
